...
void (interrupt far *irq_SetHandler
     (int iIRQ,void (interrupt far *Handler)()) ) ()
...
//  if (argc < 2) abort();
 ...
 so groá wie notwendig angelegt und nur die Haelfte ohne Grenze benutzt */
  ...
  ll1 = (( long)FP_SEG(fpBuff) << 4L) + (long)FP_OFF(fpBuff);
  fpBuff = (void far *)(((ll1 & 0xf0000) << 12) + (ll1 & 0xffff));  /* Environmentvar. BLASTER auswerten*/
  Env = getenv("BLASTER");
  if (!cp)
   {
  ...
  if (dsp_Reset())
   {
    printf("Error bei DSP-Reset\n");
    exit(99);
   }
  err = _dos_open("TADA.WAV", O_RDONLY, &handle);
  if (err)
   {
    perror(argv[1]);
    exit(0);
     ...
     switch(regs.h.al)
      {
       ...
       case '+': lseek(handle,+1764000L,SEEK_CUR);/* 10 Sek vor */
                 break;
       case '-': lseek(handle,-1764000L,SEEK_CUR);/* 10 Sek zur. */
                 break;
...
void start_dma_dsp(void)
 { /* int DMA-Splitbuffering */
  BufCnt = 0; irqs = 0;
  lseek(handle, 44L, SEEK_SET); /* Skip Header der WAVE-Datei */
  read(handle, pBuffer0, DMABUFSIZE); /*Beide Halbbuffer fuellen */
  ...
  dsp_Write(LOBYTE(44100));  /* DAC+AUTOINIT+USEFIFO+16bit-DMA */
  cmd  = 4 | 2 | 0xb0;  /* stereo + signed */
  dspmode  = 0x20 | 0x10;  /* Kommando, Modus und Laenge an DSP*/
  dsp_Write(cmd);
  dsp_Write(dspmode);      /* Halber Buffer in WORDS bzw. BYTES*/
...
void dma_SetChannel(int Chan, void far * mem, WORD Size, BYTE Mode)
 {
  ...
  /*DMA uebertr.1 Byte mehr als Size!*/
  /* lineare 20-Bit Adresse erzeugen */
  if(Chan <= 3) /* 8Bit DMA */
...
void interrupt far dsp_IrqHandler()
 ...
 /* alle DSP-Versionen vor V4.xx */
 /* nur 1 Verursacher moeglich */
 /*   inp(DspPort + 0x0e); */
  /* Interruptkontroller das Ende des IRQ's signalisieren */
...
void irq_onoff(int irq,int on)
 {
  int Port; /* Port des zustaendigen 8259 ermitteln */
   ...
   Port = (irq <= 7) ? MASTER_8259+1 : SLAVE_8259+1 ;
   irq &= 0x0007;/* untere 3 bit waehlen Kanal des entsprechenen 8259 */
   if (on) outp(Port, inp(Port) & (1 << irq)); /* Bit Loeschen: Interrupt erlaubt */
