...
**Objektmodell (`Core/Objects/`):** Jedes Ding im Cave ist eine Klasse — `CaveObject` als abstrakte Basis (Identität, Verarbeitet-Flag, Animationsphase, Aussehen, Physik-Prädikate), darunter die konkreten Objekte. Zwei Zwischenklassen bündeln, was sich zwei Objekte wirklich teilen: `FallingObject` (Stein/Diamant: Fall-Momentum, Abrollen, Landeklang, Zaubermauer-Umwandlung) und `CreatureObject` (Geist/Schmetterling: Blickrichtung, Drehsinn, Explosionsart). `CaveObjects` ist die einzige Stelle, die eine Element-ID auf ihre Klasse abbildet.

Das Cave-Gitter (`Cave`) hält diese Objekte, nicht mehr rohe Bytes. Im Original steckten Element-ID und Zustand in den Bits *eines* Bytes (0x0F ID, 0x80 verarbeitet, 0x40 Fall-Momentum, 0x60 Blickrichtung) — weil Stein und Kreatur sich dasselbe Byte teilten, überlappten Momentum- und Richtungsbits. Mit eigenen Feldern je Typ ist das gegenstandslos. `CaveObject.ToRaw()` rechnet den Zustand noch ins Originalbyte zurück, aber nur für Serialisierung und Regressionsvergleiche; die Physik liest keine Bytes mehr. **Die Bytekodierung gilt nur noch im Dateiformat** (`CaveData.Tiles` bleibt `byte[]` — es ist der unveränderliche Dateiinhalt und wird zwischen Durchläufen wiederverwendet).

Auch die **Animation** gehört den Objekten: Die früheren globalen Zähler (`wechsel_vier`, `wechsel_explo`, `wechsel_boulder`) sind weg, jedes Objekt führt seinen eigenen (`AnimationPhase`, `ExplosionObject.ExplosionPhase`, `RockfordObject`). Sie laufen dennoch **synchron** — alle Diamanten einer Cave zeigen dasselbe Bild —, weil `Cave.Animate()` sie im selben Takt schaltet und frisch entstehende Objekte mit `Cave.AnimationPhase` geboren werden. Erst wenn Objekte künftig unabhängig animieren sollen, laufen sie auseinander; das wäre eine bewusste Verhaltensänderung. Dasselbe gilt für die Explosionen: Ihr Zähler war eine globale Variable, die *jede neue Explosion auf 1 zurücksetzte* — auch für anderswo schon halb abgelaufene, weshalb alle gemeinsam verschwanden. `CavePhysics.Explode` tut das weiterhin ausdrücklich für alle (`RestartExplosions`).
...
- **Objects/** — die Spielobjekte selbst (siehe Objektmodell oben): `CaveObject` plus die 16 konkreten Klassen, `CaveObjects` (Registry), `TileAppearance`/`RenderContext` (der MonoGame-freie Zeichenauftrag eines Objekts an die Rendering-Schicht). Ein Objekt bringt alles mit, was es ausmacht: Kartenglyphe, Sprite-Frames, Verhaltens-Prädikate — früher lag das über vier Tabellen und zwei `switch`-Blöcke verstreut.
- **Simulation/** — der Kern: `GameTick` ist das Äquivalent der Original-Timer-ISR und treibt pro Tick `Clocks` (clk_1/4/18-Zähler), `Cave.Animate()` (die Animation aller Objekte), `CavePhysics` (regel(): fallende Steine/Diamanten, Gegner, Amoeba, Explosionen) und `Camera`. `Cave` ist das Gitter aus `CaveObject`s. Der Zufall (Amoeba-Wachstum, Steinschub, Rockfords Ruheanimation, `ScreenCover`) kommt aus einem `System.Random` mit festem Seed, das `GameSession` durchreicht — damit bleiben Demo und Golden-State deterministisch; **die Reihenfolge und Anzahl der Ziehungen ist deshalb verhaltensrelevant**. `Rgb`/`Palette` halten nur noch den Farbtyp und die Blitzfarben des Ausgangs — die Cave-Farben selbst stehen als RGB in der Cave-Datei.
