...
**Die Objekte rechnen sich selbst aus.** Jedes kennt seine `Cave` (Konstruktor) und seinen `Index` darin, kann also seine Nachbarn ansehen, sich bewegen, sammeln und sprengen. Die Cave gibt nur den Takt: `Cave.NextFrame()` je Tick (Animation), `Cave.NextState()` je Cave-Scan (Physik) — beide rufen reihum `NextFrame()`/`NextState()` auf jedem Objekt auf. **Die Spielregeln stehen damit im jeweiligen Objekt:** Der Stein weiß, wie er fällt, die Kreatur, wie sie an der Wand entlangläuft, Rockford, wie er gräbt und schiebt. Eine eigene Physik-Klasse gibt es nicht mehr (`CavePhysics` ist aufgelöst). In `Cave.NextState()` bleibt nur, was kein einzelnes Objekt entscheiden kann: Rockfords Todeserkennung und die Amoeba, die ihr Schicksal aus der Zählung des **gesamten** vorigen Scans zieht (`Cave.ReportAmoeba`/`Cave.AmoebaFate`).

Damit ein Objekt das kann, ist **die `Cave` die Welt**: Sie hält neben dem Gitter auch `GameState` (Punkte, Quote, Restzeiten, Sound-Ereignisse), `InputState`, `Camera` und den `Random`. Wer keine Regeln hat (Erde, Wände, Türen), erbt einfach das leere `NextState()`.

Die Scan-Richtung ist verhaltensrelevant: Weil zeilenweise von oben nach unten gescannt wird, würde ein fallendes Objekt seiner eigenen Bewegung hinterherfallen — das verhindert das Verarbeitet-Flag (`CaveObject.Scanned`), das jedes Objekt beim Zug setzt und der Scan am Ende löscht.
